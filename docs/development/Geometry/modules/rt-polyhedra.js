...
export const Polyhedra = {
    ...
    rhombicDodecahedron: (halfSize = 1) => {
      ...
      console.log(`[ThreeRT] Rhombic Dodecahedron RT construction (dual of cuboctahedron):`);
      ...
      console.log(`  Rhombic dodec octant vertices: u = t/2 = s/(2√2) = ${u.toFixed(6)}`);
      ...
      const validation = RT.validateEdges(vertices, edges, expectedQ);
      const maxError = Math.max(...validation.map(v => v.error));
      console.log(`Rhombic Dodecahedron (dual): Expected edge Q=${expectedQ.toFixed(6)} (= s²/2), Max error=${maxError.toExponential(2)}`);
    ...
    cuboctahedron: (halfSize = 1) => {
      ...
      console.log(`[ThreeRT] Cuboctahedron (Vector Equilibrium) RT construction:`);
      console.log(`  HalfSize: s = ${s.toFixed(6)}`);
      console.log(`  √2 = ${sqrt2.toFixed(6)} (deferred expansion)`);
      console.log(`  Vertex distance from origin: s/√2 = ${t.toFixed(6)} (rationalized!)`);
      ...
      const edges = [
        ...
        [0, 4], [0, 5], [0, 8], [0, 9],   // From vertex 0
        [1, 4], [1, 5], [1, 10], [1, 11], // From vertex 1
        [2, 6], [2, 7], [2, 8], [2, 9],   // From vertex 2
        [3, 6], [3, 7], [3, 10], [3, 11], // From vertex 3
        // Edges between XZ and YZ plane vertices
        [4, 8], [4, 10],   // From vertex 4
        [5, 9], [5, 11],   // From vertex 5
        [6, 8], [6, 10],   // From vertex 6
        [7, 9], [7, 11]    // From vertex 7
      ];
      ...
      const validation = RT.validateEdges(vertices, edges, expectedQ);
      const maxError = Math.max(...validation.map(v => v.error));
      console.log(`Cuboctahedron: Expected edge Q=${expectedQ.toFixed(6)}, Max error=${maxError.toExponential(2)}`);
