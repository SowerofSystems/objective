...
TEUI.FieldManager = (function () {
  ...
  /**
   * ✅ GERMAN LOCALIZATION: Country-specific field definitions
   * This registry overrides Section field definitions with German building
   * codes, standards, and occupancy types for the German market.
   *
   * NOTE: This is the ONLY section that differs from Canadian FieldManager.js
   * All other functions (1600+ lines) are identical.
   */
  const fieldDefinitions = {
    d_12: {
      fieldId: "d_12",
      type: "dropdown",
      value: "A-Assembly", // Same as Canadian (universal occupancy types)
      options: [
        // NOTE: Currently using same occupancy types as Canadian version
        // German building codes use similar classifications (Gebäudeklassen)
        // Future enhancement: Map to German Musterbauordnung (MBO) categories
    ...
    d_13: {
      fieldId: "d_13",
      type: "dropdown",
      value: "DIN V 18599 Neubau", // German default: GEG new construction standard
      options: [
        // German Climate Zone Standards (DIN 18599)
        ...
        // GEG (Gebäudeenergiegesetz) Standards
        { value: "DIN V 18599 Neubau", name: "GEG Neubau (DIN 18599)" },
        { value: "DIN V 18599 Sanierung_Bestand", name: "GEG Sanierung (Bestand)" },

        // Passivhaus Institut Standards
        ...
        { value: "EnerPHit", name: "EnerPHit (Sanierung)" },
        { value: "PH Low Energy", name: "PH Low Energy" },

        // Custom Standard Option
        { value: "ADD YOUR OWN HERE", name: "Eigener Standard" },
      ],
    },

    // Additional fields will be added in Phase 0.2:
    // d_19: { ... }, // Bundesland (Federal State)
    // h_19: { ... }, // Stadt (City)
  };
  ...
  function initializeDropdownsFromFields(sectionId) {
    ...
    Object.entries(fields).forEach(([fieldId, field]) => {
      if (field.type === "dropdown" && field.dropdownId) {
        ...
        // Add "Select..." placeholder option at the beginning (German)
        const placeholderOption = document.createElement("option");
        placeholderOption.value = "";
        placeholderOption.textContent = "Standard wählen";
        placeholderOption.disabled = true;
        // Only select placeholder if no valid default exists
        ...
        options.forEach(option => {
          ...
          // Select default value if it matches (check field.value, not field.defaultValue)
        ...
        // Force dropdown to show default value visually (fixes browser rendering issue)
        if (field.value) {
