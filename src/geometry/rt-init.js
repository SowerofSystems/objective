      ...
      function startARTexplorer(
        ...
        function createIVMPlanes() {
          ...
          const tessellations = sliderElement
        ...
        function renderPolyhedron(group, geometry, color, opacity) {
          ...
          if (showNodes) {
            ...
            PerformanceClock.timings.lastNodeTriangles =
        ...
        function updateGeometryStats() {
          ...
          if (document.getElementById("showGeodesicTetrahedron").checked) {
            ...
            const projection = projectionRadio ? projectionRadio.value : "out";
            const geodesicTetra = Polyhedra.geodesicTetrahedron(1, isNaN(frequency) ? 1 : frequency, projection);
            const eulerOK = RT.verifyEuler(
          ...
          if (document.getElementById("showGeodesicOctahedron").checked) {
            ...
            const projection = projectionRadio ? projectionRadio.value : "out";
            const geodesicOcta = Polyhedra.geodesicOctahedron(1, isNaN(frequency) ? 1 : frequency, projection);
            const eulerOK = RT.verifyEuler(
          ...
          if (document.getElementById("showGeodesicIcosahedron").checked) {
            const frequency = parseInt(document.getElementById("geodesicIcosaFrequency").value);
            const projectionRadio = document.querySelector('input[name="geodesicIcosaProjection"]:checked');
            const projection = projectionRadio ? projectionRadio.value : "out";
            const geodesicIcosa = Polyhedra.geodesicIcosahedron(1, isNaN(frequency) ? 1 : frequency, projection);
            const eulerOK = RT.verifyEuler(
        ...
        document
          .getElementById("quadrayTessSlider")
          .addEventListener("input", e => {
        ...
        function switchCameraType(toOrthographic) {
          ...
          if (toOrthographic && !isOrthographic) {
            ...
            console.log(
              "✅ Switched to Orthographic camera (parallel projection)"
            );
        ...
        document
          ...
          .addEventListener("click", function () {
            ...
            if (selected.length === 0) {
              console.warn(
                "⚠️ No polyhedra selected - cannot deposit instance"
              );
              return;
            }
            ...
            console.log(
              `🏠 Forms reset to origin - ready for next transformation`
            );
          });
        ...
        function clearHighlight(polyhedron) {
          polyhedron.traverse(obj => {
            if (obj.isMesh && obj.userData.originalEmissive) {
              obj.material.emissive.copy(obj.userData.originalEmissive);
              obj.material.emissiveIntensity =
                obj.userData.originalEmissiveIntensity;
            } else if (
              obj.isLine &&
              obj.userData.originalLineWidth !== undefined
            ) {
              ...
              if (currentPoint) {
                ...
                if (currentGumballTool === "move") {
                  ...
                  if (selectedHandle.type === "uniform") {
                    ...
                    const direction = movement.dot(
                      currentPoint.clone().normalize()
                    );
                  ...
                  if (selectedPolyhedra.length > 0) {
                    selectedPolyhedra.forEach(poly => {
                      ...
                      const newScale =
                        poly.userData.currentScale * scaleMultiplier;

                      // Clamp scale to reasonable bounds (0.1 to 10.0)
                      const clampedScale = Math.max(
                        0.1,
                        Math.min(10.0, newScale)
                      );
                    ...
                    if (
                      ...
                      const currentScale =
                        selectedPolyhedra[0].userData.currentScale;
                      const cubeSlider = document.getElementById("scaleSlider");
                      const tetSlider =
                        document.getElementById("tetScaleSlider");
                  ...
                  const snappedAngleDegrees =
                    (snappedAngleRadians * 180) / Math.PI;
                  ...
                  */
                  ...
                  if (selectedPolyhedra.length > 0) {
                    selectedPolyhedra.forEach((poly, index) => {
                      ...
                      poly.position.copy(
                        rotationCenter.clone().add(rotatedOffset)
                      );
        ...
        // Initialize selection click listener
        ...
        document.addEventListener("keydown", event => {
          ...
          if (event.key === "Delete" || event.key === "Backspace") {
            if (currentSelection && currentSelection.userData.isInstance) {
              ...
              currentSelection &&
              !currentSelection.userData.isInstance
            ) {
              console.warn(
                "⚠️  Cannot delete Forms (templates), only Instances"
              );
            }
          }
